...
const ComputerControlOrchestrator = require("./services/computerControlOrchestrator");
const computerControlConfig = require("./config/computerControlConfig");
...
wss.on("connection", (ws) => {
  ...
  // Initialize computer control orchestrator
  const orchestrator = new ComputerControlOrchestrator(computerControlConfig, gptKey);
  ...
  gptClient.on("open", function open() {
    ...
    // Enable input audio transcription for computer control
    const sessionConfig = {
      type: "session.update",
      session: {
    ...
    gptClient.send(JSON.stringify(sessionConfig));
  ...
  gptClient.on("message", async (data) => {
    const parsedData = JSON.parse(data);
    console.log(parsedData.type);

    // Handle transcript for computer control
    if (parsedData.type === "conversation.item.input_audio_transcription.completed") {
      const transcript = parsedData.transcript;
      if (transcript) {
        console.log("User said:", transcript);
        
        // Process command for computer control
        try {
          const result = await orchestrator.processCommand(transcript, sendToClient);
          if (result.handled) {
            console.log("Computer control handled command:", result);
          }
        } catch (error) {
  ...
  ws.on("message", (message) => {
    try {
      ...
      // Handle computer control specific messages
      if (event.type === "computer_control_confirmation") {
        // User responded to a confirmation request
        orchestrator.handleConfirmationResponse(event.confirmationId, event.approved);
        return;
      }

      if (event.type === "computer_control_toggle") {
        // Toggle computer control mode
        const result = orchestrator.setComputerMode(event.enabled);
        sendToClient({
          type: "computer_control_mode",
          ...result
        });
        return;
      }

      if (event.type === "computer_control_describe_screen") {
        // Request screen description
        orchestrator.describeScreen().then(description => {
          sendToClient({
            type: "screen_description",
            description
          });
        }).catch(error => {
          console.error("Screen description error:", error);
        });
        return;
      }

      if (event.type === "computer_control_get_history") {
        // Get action history
        const history = orchestrator.getHistory();
        sendToClient({
          type: "computer_control_history",
          history
        });
        return;
      }

      // Forward other messages to GPT
